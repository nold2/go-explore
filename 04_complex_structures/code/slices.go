...
func main() {
	...
	splicedFruit := fruitArray[2:4]
	fmt.Println(splicedFruit)
	fmt.Println(len(splicedFruit))
	fmt.Println(cap(splicedFruit))

	fruidToAdd := append(splicedFruit, "canteloupe", "cherries", "lemon")

	fmt.Println(fruidToAdd)
	fmt.Println(len(fruidToAdd))
	fmt.Println(cap(fruidToAdd))
	// ***************************
